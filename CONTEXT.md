...
convex/                Convex backend
  ...
  photos.ts            queries: list, getBySlug · mutations (auth-gated via assertAdmin → Convex Auth + ADMIN_EMAILS): generateUploadUrl, create, update, remove, reorder.
...
> `MIGRATION_TOKEN` and the `GOOGLE_*` vars were only ever needed for the one-time Google-Sheet→Convex import, which is done; the import script was removed on 2026-05-13, and the matching bypass code in `convex/photos.ts` was deleted in the cleanup pass later the same day (see §11). If `MIGRATION_TOKEN` is still set on any deployment, remove it (`npx convex env remove MIGRATION_TOKEN`) — it's a no-op now, just hygiene.
...
- **`/admin` auth (post-hardening):** now Convex Auth (email+password) + an `ADMIN_EMAILS` allow-list for write authorization — no app secret in the client. Self-service sign-up is disabled (no UI affordance, and `convex/auth.ts`'s `Password({ profile })` rejects `signUp` for any email not on `ADMIN_EMAILS`); the admin account is created via `npx convex run setup:createAdmin`. Residual worth knowing: the session JWT lives in `localStorage` by default (Convex Auth's standard; short-lived & revocable — switch to `storage: "inMemory"` on the server provider if you want). The `migrationToken` bypass that used to live alongside this was deleted on 2026-05-13 — see §11.
...
User wanted the website icon to be `favicon.ico`, not the dynamic "rk" lowercase mark generated by `src/app/icon.tsx`. Deleted both `src/app/icon.tsx` (32×32 tab favicon) and `src/app/apple-icon.tsx` (180×180 iOS home-screen "RK / PAI" mark). Moved `public/favicon.ico` → `src/app/favicon.ico` so Next auto-emits the `<link rel="icon">` from the file-convention. The supplied .ico has three frames (16/32/48); used the 48×48 frame as the upscale source.

Generated three static PNGs with `ffmpeg -map 0:2 -vf "scale=N:N:flags=lanczos"`:
- `src/app/apple-icon.png` (180×180) — auto-wired by Next as `<link rel="apple-touch-icon">`.
- `public/icons/icon-192.png` and `public/icons/icon-512.png` — the PWA-manifest standard sizes.

Updated `src/app/manifest.ts`'s `icons` array to reference the two static PNGs (was pointing at the now-deleted `/icon` and `/apple-icon` routes). 

**Caveat:** the source .ico tops out at 48px, so the 180/192/512 upscales look noticeably soft — fine for a tab favicon (the browser uses the 32×32 frame from the .ico directly) but visibly low-res on iOS home-screen. A hi-res master (256+ or SVG) would fix that; out of scope for this pass.

### 2026-05-13 — Group #2 fixes: SlideShow crossfade + double-fetch + footer transitions

Three correctness/UX bugs from the audit, batched.

**SlideShow now consumes its `photos` prop.** The landing-page `<SlideShow>` had been passed `photos.slice(0, 10)` from `src/app/page.tsx` since the Convex migration, but `PortfolioGrid` quietly ignored it — every tile used hard-coded `/public/images/birdN.jpg` paths. Worse, every crossfade re-mounted the `<motion.div>` (key={src}) inside `AnimatePresence`, so `next/image` re-started its decode from scratch, and during the transition the `bg-stone-200` site background leaked through the dim overlay — that was the "gray placeholder" bug. Rewrote it: all slides for a tile stay mounted as `absolute inset-0` siblings, only their opacity flips, so the next image is already decoded before it becomes the visible one. Round-robin distributes 10 Convex photos across 4 tiles (so each tile gets a variety, not a sequential clump), deterministic per-tile rotation every 5s with a 1.25s phase offset per tile (no more `Math.random() > 0.5` no-ops), `requestIdleCallback` preload for non-priority slides as a slow-connection belt, and a `bg-stone-300` on the tile itself as a safety net. Captions pull from `photo.location` or a 79-char trim of `photo.story`. Falls back to the static `/public/images/birdN.jpg` set when fewer than 4 usable photos are available (build before `convex dev` / empty gallery). Dropped framer-motion from this file — Tailwind opacity transitions handle the crossfade natively (GPU-composited, no reconciliation cost), and the `@ts-nocheck` came off too.

**Stale Google-Sheets comments in [src/app/page.tsx](src/app/page.tsx).** "Changed from googleSheets to the main photo module" / "fetchPhotos function that now uses Google Sheets" — both were lies (Convex has been the source since the migration). Removed; also slimmed the projects-shape map (it was carrying `story` + `createdAt` fields that `<Projects>` doesn't consume) and dropped the file's `@ts-nocheck` (the simpler body type-checks cleanly).

**[src/app/stories/[id]/page.tsx](src/app/stories/[id]/page.tsx) was fetching twice per render.** `generateMetadata` calls `fetchPhotoBySlug(id)` (one Convex by-index query), the page body was then doing `fetchPhotos()` (full `.collect()`) and `.find()`-ing in JS — a full-table scan to find one row. Replaced with a second `fetchPhotoBySlug(id)`; both calls dedupe over a single request via Next's per-request cache, so this is effectively one network round-trip now.

**Footer links now use the page-transition.** Four nav links in [src/sections/Footer.tsx](src/sections/Footer.tsx) (`Home`, `Album`, `Gallery`, `Recent Stories`) were plain `<a href>` tags — they bypassed the curtain wipe entirely (jarring full-page reload). Wired them through `useStartPageTransition` + `router.push` like `Header.tsx` does. While there: changed `Home` from `#` (scrolled to top of doc on any page) to `/` (actually goes home), and changed `Recent Stories` from `#projects` to `/#projects` so it works from `/stories` / `/album` (the fragment-only anchor was a no-op there — no `#projects` element exists off the home page). Used `window.location.assign(href)` for the cross-route hash hard-nav so the new `react-hooks/immutability` lint rule doesn't fire (Header.tsx still uses the property-assignment form, masked by its blanket `eslint-disable` — that's group #3 territory).

Verification: `pnpm run lint` → 0 errors, 1 pre-existing unrelated warning; `npx tsc --noEmit` → clean.

### 2026-05-13 — Security + dead-code cleanup: removed the `migrationToken` auth bypass

Closed the loophole that §9/§11 had been flagging as "delete this". Before: `convex/photos.ts` exported an `assertAdminOrMigration` helper that let any unauthenticated caller invoke `generateUploadUrl`/`create` if they presented a `migrationToken` matching `process.env.MIGRATION_TOKEN` — a real auth bypass kept alive only for the (already-deleted) Google-Sheet import script. After: the helper is gone, both mutations call `assertAdmin(ctx)` directly, and the `migrationToken` arg has been removed from both function signatures.

Confirmed the bypass was dead in practice: `npx convex env list` shows no `MIGRATION_TOKEN` set on the current deployment, so no env-cleanup step is needed. No callsites in the repo passed the arg either (the import script that did was deleted earlier today).

Same pass also cleared 12 of the 13 ESLint warnings — all `Unused eslint-disable directive` (the `next/typescript` preset doesn't include `@typescript-eslint/no-explicit-any` or `@typescript-eslint/no-unused-vars` as configured, so the blanket `/* eslint-disable */` lines at the top of the older files weren't suppressing anything). Removed from: `src/lib/photo.ts`, `src/app/page.tsx`, `src/app/stories/page.tsx`, `src/app/album/page.jsx`, `src/app/admin/page.tsx`, `src/components/GalleryGrid.tsx`, `src/components/Story.tsx`, `src/sections/About.tsx`, `src/sections/Testimonials.tsx`, `tailwind.config.ts`. **`// @ts-nocheck` lines were kept** — those are doing real work (the files still aren't typed), and tightening them is the next planned PR (the `Photo`-shape cleanup). Also fixed `convex/auth.config.ts` to assign its config to a named variable before exporting (the lone `import/no-anonymous-default-export` warning). Remaining warning: one `react-hooks/exhaustive-deps` in the shadcn `ui/typewriter-effect.tsx` primitive — out of scope for this pass.

Verification: `pnpm run lint` → 0 errors, 1 unrelated warning; `npx tsc --noEmit` → clean.
